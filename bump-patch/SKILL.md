...
name: bump-patch
description: >
  Increment patch version (e.g. 1.1.2 -> 1.1.3) across all project files.
  Trigger on "/bump-patch", "bump patch".
...
Increment the **patch** version across all project files. No questions — just do it.
...
4. Bump patch: `major.minor.patch` → `major.minor.(patch+1)`
...
```
✅ bump-patch: 1.1.2 → 1.1.3 (build 46 → 47)

| Arquivo         | Campo                      | Antes | Depois |
|-----------------|----------------------------|-------|--------|
| package.json    | version                    | 1.1.2 | 1.1.3  |
| app.json        | expo.version               | 1.1.2 | 1.1.3  |
